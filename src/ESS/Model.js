...
class Model extends BaseModel.Inverter {
  ...
  reload() {
    ...
    this.BASE.pvKwh =
      _.multiply(_.multiply(this.BASE.pvAmp, this.BASE.pvVol), 0.001) * _.random(10, 15);
    ...
    this.BASE.batteryChargingKw = _.round(
      _.multiply(_.multiply(this.BASE.batteryVol, this.BASE.batteryAmp), 0.001),
      4,
    );
    this.BASE.batteryDischargingKw = _.round(
      _.multiply(_.multiply(this.BASE.batteryVol, this.BASE.batteryAmp), 0.001),
      4,
    );
    this.BASE.batteryTotalChargingKw = _.round(
      _.multiply(this.BASE.powerCpKwh, _.random(0.8, 0.99)),
      3,
    );
    this.BASE.totalPVGeneratingPowerKwh = _.round(
      _.multiply(this.BASE.powerCpKwh, _.random(0.8, 0.99)),
      3,
    );
    this.BASE.ledDcVol = _.random(160.1, 190.1);
    this.BASE.ledDcAmp = _.random(0.3, 7.7);
    this.BASE.ledUsingKw = _.round(
      _.multiply(_.multiply(this.BASE.ledDcVol, this.BASE.ledDcAmp), 0.001),
      4,
    );
